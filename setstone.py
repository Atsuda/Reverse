...
class Setstone():
	...
	def __init__(self):
		...
		self.down_vec = []
		self.rightDown_vec = []
		vector_list = {1: self.right_vec, 2: self.rightUp_vec, 3: self.Up_vec, 4: self.leftUp_vec, 5: self.left_vec,
		               6: self.leftDown_vec, 7: self.down_vec, 8: self.rightDown_vec}
		# next_stoneメソッドで取得するリストの入れ物
		self.next_stone_vector_option = []
	...
	def next_stone(self, playernum):
		...
		for num in range(1, 9):
			if Setstone.vector_list[num][1] == playernum * (-1):
				self.next_stone_vector_option.append(num)
			else:
				pass
		if self.next_stone_vector_option == []:
			print 'er2'

	def can_reverse_stone(self, playernum):
		# 連続する相手の石の次が自分の石であるかの判定
		for num in self.next_stone_vector_option:
